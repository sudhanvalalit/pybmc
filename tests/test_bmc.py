...
class TestBayesianModelCombination(unittest.TestCase):
    ...
    def test_orthogonalize(self):
        self.bmc.orthogonalize(property=self.property, train_df=self.train_df, components_kept=2)
    ...
    def test_train(self):
        self.bmc.orthogonalize(property=self.property, train_df=self.train_df, components_kept=2)
        self.bmc.train()
        self.assertEqual(self.bmc.samples.shape[1], 3)  # 2 components + 1 sigma
    ...
    def test_predict(self):
        ...
        rndm_m, lower_df, median_df, upper_df = self.bmc.predict2(self.property)
