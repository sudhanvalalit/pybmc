...
class TestDataset(unittest.TestCase):
    ...
    def test_load_data_csv(self, mock_read_csv, mock_exists):
        ...
        result = dataset.load_data(models=["modelA", "modelB"], keys=["target"], domain_keys=["x", "y"], model_column="model")

        self.assertIn("target", result)
        self.assertTrue(all(col in result["target"].columns for col in ["x", "y", "modelA", "modelB"]))
    ...
    def test_load_data_h5(self, mock_read_hdf, mock_exists):
        ...
        result = dataset.load_data(models=["modelA", "modelB"], keys=["target"], domain_keys=["x", "y"])
    ...
    def test_split_data_inside_to_outside(self):
        ...
        self.assertTrue(any((row["x"] == 1 and row["y"] == 1) for _, row in train.iterrows()))

    def test_get_subset_basic_filter(self):
        filters = {"x": lambda x: x > 2}
        result = self.dataset.get_subset(property_name="target", filters=filters, models_to_include=["modelA", "modelB"])
        self.assertIsInstance(result, pd.DataFrame)
        self.assertTrue((result["modelA"].notna()).all())  # Check filtered rows exist
        self.assertTrue(all(col in result.columns for col in ["modelA", "modelB"]))
